...
public class BDController {
    ...
    public void insertarVerbo(String esp, String present, String past, String pastParticipie, int codUser, String type){
        String sql = "insert into verbos values ('"+esp+"','"+present+"','"+past+"','"+pastParticipie+"',"+codUser+",'"+type+"');";
        try {
            Statement ms = this.connection.createStatement();
            ms.executeUpdate(sql);
            ms.close();
    ...
    public ArrayList<String> wordsEng(){
        ArrayList<String> words = new ArrayList<String>();
        try {
            Statement ms = this.connection.createStatement();
            ResultSet rs = ms.executeQuery("SELECT word from ding");
            while (rs.next()){
             words.add(rs.getString(1).substring(0,rs.getString(1).length()-1));
            }
        }catch (SQLException e){
            e.printStackTrace();
        }
    ...
    public void insertWord2(){
        ArrayList<String> words = wordsEng();
        try {
            Statement ms = this.connection.createStatement();
            ms.executeUpdate("delete from ding");
            for (String palabra : words) {
                ms.executeUpdate("insert into ding values ('"+palabra+"') ");
            }
        }catch (SQLException e){
            e.printStackTrace();
        }
